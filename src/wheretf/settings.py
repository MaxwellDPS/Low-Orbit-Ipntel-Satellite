...
WSGI_APPLICATION = 'wheretf.wsgi.application'
...
# IDK where ist gonna be run, but im happy to provide a cname on panik.io :)
if DEBUG:
    ...
    CORS_ALLOWED_ORIGINS = os.getenv("CORS_ALLOWED_HOSTS", "127.0.0.0").split(" ")
...
if STATIC_USE_S3:
    ...
    STATICFILES_STORAGE = "wheretf.storage_backends.StaticStorage"
else:
    STATIC_URL = "/static/"
...
if STATIC_USE_S3:
    # STORE A FILE 🗂️
    DEFAULT_FILE_STORAGE = "wheretf.storage_backends.PrivateMediaStorage"
    GEOIP_LOCATION = "geoip"
    GEOIP_URL = (
        f"https://{AWS_S3_CUSTOM_DOMAIN}/{AWS_STORAGE_BUCKET_NAME}/{GEOIP_LOCATION}/"
    )
    GEOIP_STORAGE = "wheretf.storage_backends.GeoIPMediaStorage"
    GEOIP_PATH = None
else:
...
if not Group.objects.filter(name='geo-sync-admins'):
    Group(
      name="geo-sync-admins"
    ).save()

GEO_SYNC_USERAGENT = "wheretf/api v0.69"
# YOU THINK YOU DID SOMETHING STUPID... Youre proably right 🫰👉
try:
    LOCAL_SETTINGS
except NameError:
    try:
        from wheretf.settings_local import *
    except ImportError:
        pass
